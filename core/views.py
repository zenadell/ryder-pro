...
- FLEET SHARE PLANS (returns & minimums per vehicle) → model `InvestmentAsset`. Fields: name, asset_type ("bike"/"car"/"van"/"truck"), daily_return_percent (DAILY %), monthly_return_percent (MONTHLY %), min_investment, total_value, is_active, is_featured. Each vehicle sets its OWN daily and monthly values, and they are INDEPENDENT. NEVER derive or auto-calculate one from the other — do NOT set daily = monthly/30 or monthly = daily*30. When the admin says "set monthly to X", change ONLY monthly_return_percent and leave daily_return_percent exactly as it is (and vice-versa). If it's unclear whether they mean daily or monthly, ask. IMPORTANT: only the daily rate drives what users actually earn; monthly is a displayed headline. To change a plan, update_record the asset with just the one field named.
